...
void rotate_90_degrees(int** matrix, int N) {
    // Untuk rotasi 90 derajat, kita melakukan transpose dan kemudian membalikkan setiap baris
    // Langkah pertama: Transposisi matriks
    transpose_nxn_avx2(matrix, N);

    // Langkah kedua: Membalikkan setiap baris matriks
    for (int i = 0; i < N; i++) {
        int start = 0;
        int end = N - 1;
        // Tukar elemen-elemen baris dari kedua ujung
        while (start < end) {
...
int main(void) {
    ...
    // Melakukan rotasi matriks 90 derajat
    rotate_90_degrees(matrix, N);

    // Mencetak matriks setelah rotasi
    printf("Matriks setelah rotasi 90 derajat:\n");
    print_matrix(matrix, N);
